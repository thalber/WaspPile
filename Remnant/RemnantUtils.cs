...
namespace WaspPile.Remnant
{
    internal static class RemnantUtils
    {
        ...
        {
            Color res = default;
            for (int i = 0; i < 3; i++) res[i] = bcol[i] + dbound[i] * URand.Range(-1, 1);
            return clamped? res.Clamped() : res;
        }

        internal static RainWorld CRW => UnityEngine.Object.FindObjectOfType<RainWorld>();
        internal static CreatureTemplate GetTemp(CreatureTemplate.Type t) => StaticWorld.creatureTemplates[(int)t];
    }
}
